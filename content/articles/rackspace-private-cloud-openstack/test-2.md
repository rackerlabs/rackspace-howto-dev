...
This is specifically for sandbox and testing / development environments&ndash; configurations described here, such as: compute and cinder running on
...
releases with minor adjustments. &rsquo;m using a single NIC / network
configuration for all services.

You may also notice that instead of using the&ldquo;all-in-on&rdquo; role I
specify roles individually. The reason I do this is because the&ldquo;all-in-on&rdquo; role defaults to nova-network, does not contain heat, and
...
I&rsquo;s not quite time to run this command yet however; le&rsquo;s continue on
...
i&rsquo;s all working before I install chef.  The only requirement is for
chef-1 to have ssh pass-thru authentication to controller-1 but there is
no harm in having bi-directional pass-thru authentication if tha&rsquo;s
preferable.
...
ssh-copy-id command. In this case i&rsquo;s likely faster and easier to copy
...
openssh.  To accomplish this manually you can use these example steps:
    ...
    ssh root@controller-1&ldquo;mkdir /root/.ssh; chmod&ndash;R 400 /root/.ss&rdquo;
...
1.  Configuring (or disabling) iptables\
     -If leaving iptables enabled yo&rsquo;ll need to open several ports for
    RPC to function properly.  You can find most of these ports listed
    in the&ldquo;api setting&rdquo; tab for the&ldquo;security and acces&rdquo; menu on the
...
1.  W&rsquo;ll be using the native Linux Volume Manager to provide block
...
3.  By design, and due to the custom storage configuration nature&ndash; the
   &ldquo;cinder-volume&rdquo; volume group is not setup by the chef cinder-volume
    role and must be completed beforehand.\
      
4.  Here are a couple of convenient tips &rsquo;ve found for setting up this
    volume group.
...
-   You can still create the cinder-volumes volume group but yo&rsquo;ll need
    to either:
...
In this section w&rsquo;ll download and install the RPC cookbooks.  To
...
In this exercise w&rsquo;ll be using 4.2.2rc; the current default stable
branch is 4.2.1.
...
Since &rsquo;m installing OpenStack on top of a hypervisor that does&rsquo;t
...
Lastly, if yo&rsquo;re not accustom to running a single NIC / single subnet
...
use the&ldquo;knife environment from fil&rdquo; creation process vs. the&ldquo;knife
environment edi&rdquo; command (as some documentation refers to). The&ldquo;knife
environment edi&rdquo; approach will not retain your environment edits if
...
Before we get to work on verifying cinder, le&rsquo;s setup a few other
components for our instances.

1.  Create a keypair.

-   If you have&rsquo;t already created ssh keys on this controller node, do
    so now
...
SS&rsquo;ing to your instances .*
...
Finally w&rsquo;re ready to test our cinder service and configuration.

1.  First le&rsquo;s test the creation of a basic volume and verify.
...
3.  Next le&rsquo;s test creating a volume from a bootable image.

-   First, le&rsquo;s make sure we have an image present
...
-   Now le&rsquo;s create a bootable volume using this image and verify that
    it is set to bootable.

*TIP: It will show bootable&ldquo;fals&rdquo; at first, but after creation it will
show as&ldquo;tru&rdquo;.*
...
-   Once created le&rsquo;s boot an instance using this newly created
    bootable volume.
