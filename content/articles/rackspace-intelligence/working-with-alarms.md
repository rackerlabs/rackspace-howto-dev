...
permalink: article/working-with-alarms
type: article
created_date: '2015-07-31 18:23:21'
created_by: constanze.kratel
last_modified_date: '2015-08-27 22:0425'
...
process of creating an alarm for a check on an entity. Rackspace
Intelligence supports the following alarm types:

  Alarm type            Alarm name
  --------------------- ---------------------------------
  remote.http           Body match - string found
  remote.http           Body match - string not found
  remote.http           Connection time
  remote.http           Status code
  remote.http           SSL certificate expiration time
...
2.  In the **Monitoring** section, click the entity for which you want
    ...
    ![](/knowledge_center/sites/default/files/field/image/intelligence-alarms-entities-list.png)

    On the entity overview page, in the **Monitoring Checks** section,
    select the check for which you want to create the alarm and click on
    it.

    ![](/knowledge_center/sites/default/files/field/image/intelligence-alarms-select-check.png)
...
5.  In the **Choose Alarm Type** section, select the type of alarm, and
    ...
    you define a custom alarm script.
...
7.  In the **Alarm Settings** section, define the criteria for your
    alarm. You must define a threshold for when a Warning is issued and
    a threshold for when the situation becomes Critical. For a CPU
...
6.  In the Alarm Settings section, specify values for **Warning
    ...
    displayed for **Current value** on the left side of the section for
...
7.  Click **Test Alarm** to test the alarm.\
    ...
    Alarm** after receiving a test result of
    type **Warning** or**Critical**, a warning message appears. You can
    still create the alarm, but creating the alarm will trigger an alert
    that is associated with your selected notification plan. If the
    current value is less than the values that you have specified
    for **Warning Threshold** and **Critical Threshold**, you receive
    an **OK** result when you click **Test Alarm**.
