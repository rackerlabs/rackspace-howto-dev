...
**Note:** After you've accessed the vRouter, you should add a local user. Follow the procedure described in [Vyatta vRouter: Adding a local administrative user](/howto/vyatta-vrouter-add-a-local-administrative-user). If you are not logged in via SSH as the user *Vyatta* or as an administrative user, then access the vRouter remotely or through the console, then add a local user. Follow the procedure at [Vyatta vRouter: Add a local administrative user}()
...
[Connecting to a server using SSH on Linux or Mac OS](/howto/connecting-to-a-server-using-ssh-on-linux-or-mac-os)
...
[Generating RSA Keys With SSH - PuTTYgen](/howto/generating-rsa-keys-with-ssh-puttygen)
