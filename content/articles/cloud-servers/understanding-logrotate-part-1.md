...
logs. I&rsquo;s true! The system usually runs logrotate once a day, and when
it runs it checks rules that can be customized on a per-directory or
per-log basis.
&ldquo;Log rotatio&rdquo; refers to the practice of archiving an applicatio&rsquo;s
current log, starting a fresh log, and deleting older logs. And while
w&rsquo;re explaining things, a&ldquo;lo&rdquo; is a file where an application stores
information that might be useful to an administrator or developer - what
i&rsquo;s been doing, what errors i&rsquo;s run into, that sort of thing. So logs
are good, you just usually do&rsquo;t want to keep a ton of them around.
Tha&rsquo;s where logrotate comes in.
...
virtual server. Another practical consideration is that i&rsquo;s hard to
look for a particular event if you have a million log entries to skim
through. So on the whole i&rsquo;s a good idea to keep log files down to a
...
distributions yo&rsquo;ll find the script that runs logrotate daily at:
...
example) yo&rsquo;ll need to look into using cron to run logrotate through a
...
If you look inside that file yo&rsquo;ll see the default parameters logrotate
uses when it rotates logs. The file is nicely commented, so skim it to
see how things are set up. W&rsquo;ll talk about several of the specific
...
Tha&rsquo;s where w&rsquo;ll find most of the application-specific configuration
files.

### logrotate.d {#logrotated}

Take a look inside the directory where yo&rsquo;ll store application-specific
log settings:

    ls /etc/logrotate.d

Depending on how much yo&rsquo;ve installed on your server there may be no
...
look inside yo&rsquo;ll see an entry for various system logs along with some
commands similar to what you saw in logrotate.conf.

**NOTE:** You wo&rsquo;t actually see an entry for a syslog service on
versions of Ubuntu older than Karmic Koala (9.10). Prior to that release
the system logs were rotated by a&ldquo;savelo&rdquo; command run from the&ldquo;/etc/cron.daily/sysklog&rdquo; script.

#### Inside an application file {#inside_an_application_file}

As an example, le&rsquo;s take a look at the contents of a logrotate config
...
W&rsquo;ll look at what most of the specific directives in this file mean in
a bit, but the short version is that when logrotate runs it will check
for any files in /var/log/httpd that end in&ldquo;lo&rdquo; and rotate them, so
long as they are&rsquo;t empty. If it checks the httpd directory and does&rsquo;t
find any logfiles it wo&rsquo;t throw an error. Then it will run the command
in the&ldquo;postrotate/endscrip&rdquo; block (in this case, a command that will
tell apache to restart), but only after i&rsquo;s processed all the specified
logs.

What you do&rsquo;t see in that file are some settings you saw back in
...
run a busy web server, you may want to include a&ldquo;dail&rdquo; command in
apach&rsquo;s config block so apach&rsquo;s logs will rotate daily instead of the
default weekly rotation.

That might be more clear if we talk about what some of the more
commonly-used commands actually do in a logrotate config file. So le&rsquo;s
do that next.
...
W&rsquo;ll go over more commonly-used commands here.
...
files will contain just one of these blocks, but i&rsquo;s possible to put
...
For example, to specify all files in the directory /var/foo that end in&ldquo;.lo&rdquo;, as well as the file&ldquo;/var/bar/log.tx&rdquo;, you would set up the
block like so:
...
The&ldquo;rotat&rdquo; command determines how many archived logs will be kept
...
one (the one with&ldquo;.&rdquo; at the end, usually) will be deleted to make room
for the new archive.
...
logrotate runs (unless another condition like&ldquo;siz&rdquo; has been set).

If you want to use a time interval other than the keywords listed here
yo&rsquo;ll have to get clever with cron and a separate config file. For
example, if you wanted to rotate a particular log file hourly, you could
create a file in&ldquo;/etc/cron.hourl&rdquo; (you may need to create that
directory too) that would contain a line like:

    /usr/sbin/logrotate /etc/logrotate.hourly.conf

Then put the configuration for that hourly run of logrotate (the log
file location, whether or not to compress old files, and so on) into&ldquo;/etc/logrotate.hourly.con&rdquo;.

#### Size

You can specify a file size that logrotate will check when determining
whether or not to perform a rotation by using the&ldquo;siz&rdquo; command. The
format of the command tells logrotate what units yo&rsquo;re using to specify
the size:
...
kilobytes, the second if i&rsquo;s larger than 100 megabytes, and the third
if i&rsquo;s over 100 gigabytes. I do&rsquo;t recommend using a limit of 100G,
...
you do&rsquo;t want compressed, but still want compression to be on by
...
compressed, but not right away. With&ldquo;delaycompres&rdquo; active an archived
log wo&rsquo;t be compressed until the next time the log is rotated. This can
be important when you have a program that might still write to its old
logfile for a time after a fresh one is rotated in. Note that&ldquo;delaycompres&rdquo; only works if you also have&ldquo;compres&rdquo; in your config.

An example of a good time to use delaycompress would be when logrotate
is told to restart apache with the&ldquo;gracefu&rdquo; or&ldquo;reloa&rdquo; directive.
...
that you wo&rsquo;t lose those extra log entries when the logs are rotated.

#### Postrotate

The&ldquo;postrotat&rdquo; script is run by logrotate each time it rotates a log
specified in a config block. Yo&rsquo;ll usually want to use this to restart
...
That&ldquo;\> /dev/nul&rdquo; bit at the end tells logrotate to pipe the comman&rsquo;s
...
case, you do&rsquo;t really care about the output if everything restarted
okay.

The&ldquo;postrotat&rdquo; command tells logrotate that the script to run will
start on the next line, and the&ldquo;endscrip&rdquo; command says that the script
is done.

#### Sharedscripts

Normally logrotate will run the&ldquo;postrotat&rdquo; script every time it
...
access log and the error log will, if it rotates both, run the&ldquo;postrotat&rdquo; script twice (once for each file rotated). So if both files
...
That tells logrotate to wait until i&rsquo;s checked all the logs for that
...
none of the logs get rotated, the postrotate script wo&rsquo;t run at all.

### Where to go next {#summary}

Yo&rsquo;ve seen an overview of what logrotate does and what kind of
