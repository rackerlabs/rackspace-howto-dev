...
permalink: article/vyatta-snmp-logging
type: article
created_date: '2015-08-26 18:33:12'
created_by: rose.contreras
last_modified_date: '2016-01-06 15:3337'
...
management and logging system are built in to the vRouter:
     ...
     *sysObjectID  =  1.3.6.1.4.1.30803\
      sysDescr        = Vyatta\_version\_info*
...
IP Address of the SNMP management ystem

10.0.0.12

vRouter specific device information
...
Use the `show log` command to view logs. Following is a table showing
...
The table above shows an example of proper syntax and a specific example
using `host` as the destination, `local3` as the facility, and `infro`
as the level. The following table shows other destination, level, and
facility options.

**Note**: As a reminder (or syslog primer), the `level` variable refers
to severity level, which can be application-specific. The `facility`
variable refers to the type of program logging the message.
...
Logging to specific user&rsquo;s terminal
...
All facilities excluding &ldquo;mark&rdquo;
